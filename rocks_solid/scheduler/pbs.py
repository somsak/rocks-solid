...
class Scheduler(BaseScheduler):
    def __init__(self, conf={}, **kw):
        ...
        # PBS commands
        # reasonable default
        self.pbs_home = kw.get('pbs_home', '/usr/pbs')
        self.qstat = 'qstat'
        self.qsub = 'qsub'
        self.qdel = 'qdel'
    ...
    def hosts(self, **kw) :
        ...
        while 1 :
            ...
            if len(value) == 1 :
                ...
                host_data = {'loadavg':0.0}
            else :
                key = value[0].strip()
                value = value[1].strip()

                if key == 'Mom' :
                    host_data['name'] = value.split('.', 1)[0]
                elif key == 'pcpus' :
                    host_data['np'] = int(value)
                    host_data['slot_total'] = int(value)
                elif key == 'resources_assigned.ncpus' :
                    host_data['slot_used'] = int(value)
                elif key == 'state' :
                    if value == 'free' or value == 'job-busy' :
                        host_data['state'] = 'up'
                    else :
